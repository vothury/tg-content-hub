...
DOUBLE_CHECK_SYSTEM = """Ты — строгий выпускающий редактор. Пост подготовлен к АВТОМАТИЧЕСКОЙ публикации. Финально проверь, не пропустила ли первая модель брак.
...
Ответь строго JSON без текста вне него:
{{"approve": true | false, "note": "<если не одобрил — коротко что не так, иначе пусто>"}}"""
