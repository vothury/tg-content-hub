...
EDITABLE = [
    ...
    {"key": _k("CLEAN_FALLBACK_MODEL"), "label": "Модель очистки подписей (страховка)", "attr": "clean_fallback_model", "type": "text",
     "hint": "Сильная модель для второй попытки, если дешёвая дала непроверяемый план; пусто = модель двойной проверки."},
    {"key": _k("LLM_FALLBACK_MODELS"), "label": "Запасные модели (через запятую)", "attr": "llm_fallback_models", "type": "list",
    ...
    {"key": _k("PRICE_WATCH_ENABLED"), "label": "Цены моделей: контроль включён", "attr": "price_watch_enabled", "type": "number",
     "hint": "1 = раз в сутки сверять цены используемых моделей с каталогом OpenRouter."},
    {"key": _k("PRICE_ALERT_PCT"), "label": "Цены моделей: порог предупреждения, %", "attr": "price_alert_pct", "type": "number",
     "hint": "Насколько должна измениться цена (input или output), чтобы показать баннер."},
    {"key": _k("PRICE_WATCH_INTERVAL_HOURS"), "label": "Цены моделей: интервал проверки, часов", "attr": "price_watch_interval_hours", "type": "number",
     "hint": "Как часто опрашивать каталог OpenRouter (запрос бесплатный)."},
    {"key": _k("PRICE_HISTORY_MIN_CHANGE_PCT"), "label": "Цены моделей: порог записи истории, %", "attr": "price_history_min_change_pct", "type": "number",
     "hint": "Изменения мельче этого процента в историю не пишутся: защита от раздувания таблицы на волатильных ценах."},
    {"key": _k("PRICE_WATCH_SCOPE"), "label": "Цены моделей: разрез провайдеров", "attr": "price_watch_scope", "type": "text",
    ...
    {"key": _k("PUBLISH_MAX_MEDIA_MB"), "label": "Публикация: лимит размера медиа, МБ", "attr": "publish_max_media_mb", "type": "number",
     "hint": "Проверяется ДО отправки в Telegram: и каждый файл, и суммарный размер альбома. Лимит Bot API — 50 МБ."},
    {"key": _k("PUBLISH_MEDIA_COMPRESS"), "label": "Публикация: сжимать видео ffmpeg", "attr": "publish_media_compress", "type": "number",
     "hint": "1 = если видео больше лимита, пересобрать его под целевой размер (нагрузка на CPU; на слабом сервере лучше держать 0)."},
    {"key": _k("PUBLISH_COMPRESS_TARGET_MB"), "label": "Публикация: цель сжатия, МБ", "attr": "publish_compress_target_mb", "type": "number",
     "hint": "Целевой размер видео после сжатия (не больше лимита)."},
    {"key": _k("PUBLISH_COMPRESS_MAX_SIDE"), "label": "Публикация: макс. сторона кадра при сжатии", "attr": "publish_compress_max_side", "type": "number",
     "hint": "Например 1280 = не шире 720p. Меньше — сильнее сжатие и меньше размер."},
    {"key": _k("PUBLISH_SKIP_OVERSIZED"), "label": "Публикация: пропускать медиа больше лимита", "attr": "publish_skip_oversized", "type": "number",
     "hint": "1 = опубликовать пост без «тяжёлого» файла вместо ошибки; 0 = пост не публикуется."},
    {"key": _k("PUBLISH_STRIP_SOURCE_DECOR"), "label": "Публикация: убирать хэштеги и ссылки-тизеры", "attr": "publish_strip_source_decor", "type": "number",
    ...
    {"key": _k("CURATION_ENABLED"), "label": "Курирование: включено", "attr": "curation_enabled", "type": "number",
     "hint": "1 = принимать пересылки из каналов-приёмников с подписью-списком целевых каналов."},
    {"key": _k("CURATION_INBOX_CHANNELS"), "label": "Курирование: каналы-приёмники (через запятую)", "attr": "curation_inbox_channels", "type": "list",
    ...
    {"key": _k("PREFILTER_SELFPROMO_PATTERNS"), "label": "Префильтр: маркеры самопиара источника", "attr": "prefilter_selfpromo_patterns", "type": "list",
     "hint": "Фразы типа «нам на канал», «залил нам», «у нас на канале»: пост отсекается технически, до вызова модели."},
    {"key": _k("PREFILTER_EVENT_MARKERS"), "label": "Префильтр: маркеры анонсов мероприятий", "attr": "prefilter_event_markers", "type": "list",
     "hint": "«регистрация», «прямой эфир», «вебинар» и т.п.; срабатывает только вместе с площадкой регистрации либо датой и временем."},
    {"key": _k("PREFILTER_EVENT_DOMAINS"), "label": "Префильтр: площадки регистрации", "attr": "prefilter_event_domains", "type": "list",
     "hint": "Домены записи на мероприятия: timepad.ru, webinar.ru, zoom.us и т.п."},
    {"key": _k("PREFILTER_SENSITIVE_WORDS"), "label": "Префильтр: чувствительные слова (через запятую)", "attr": "prefilter_sensitive_words", "type": "list",
    ...
    {"key": _k("LLM_RESPONSE_LANG"), "label": "LLM: язык ответов моделей", "attr": "llm_response_lang", "type": "text",
    ...
    {"key": _k("LLM_SENSITIVE_MODEL"), "label": "LLM: модель для чувствительных постов", "attr": "llm_sensitive_model", "type": "text",
...
ATTR_TO_KEY = {
    ...
    "clean_fallback_model": _k("CLEAN_FALLBACK_MODEL"),
    "llm_fallback_models": _k("LLM_FALLBACK_MODELS"),
    "price_watch_enabled": _k("PRICE_WATCH_ENABLED"),
    "price_alert_pct": _k("PRICE_ALERT_PCT"),
    "price_watch_interval_hours": _k("PRICE_WATCH_INTERVAL_HOURS"),
    "price_history_min_change_pct": _k("PRICE_HISTORY_MIN_CHANGE_PCT"),
    "price_watch_scope": _k("PRICE_WATCH_SCOPE"),
    "publish_dup_recap_window_hours": _k("PUBLISH_DUP_RECAP_WINDOW_HOURS"),
    "publish_restore_links": _k("PUBLISH_RESTORE_LINKS"),
    "publish_max_media_mb": _k("PUBLISH_MAX_MEDIA_MB"),
    "publish_media_compress": _k("PUBLISH_MEDIA_COMPRESS"),
    "publish_compress_target_mb": _k("PUBLISH_COMPRESS_TARGET_MB"),
    "publish_compress_max_side": _k("PUBLISH_COMPRESS_MAX_SIDE"),
    "publish_skip_oversized": _k("PUBLISH_SKIP_OVERSIZED"),
    "publish_strip_source_decor": _k("PUBLISH_STRIP_SOURCE_DECOR"),
    "curation_enabled": _k("CURATION_ENABLED"),
    "curation_inbox_channels": _k("CURATION_INBOX_CHANNELS"),
    ...
    "prefilter_selfpromo_patterns": _k("PREFILTER_SELFPROMO_PATTERNS"),
    "prefilter_event_markers": _k("PREFILTER_EVENT_MARKERS"),
    "prefilter_event_domains": _k("PREFILTER_EVENT_DOMAINS"),
    "prefilter_sensitive_words": _k("PREFILTER_SENSITIVE_WORDS"),
    "llm_sensitive_model": _k("LLM_SENSITIVE_MODEL"),
    "llm_response_lang": _k("LLM_RESPONSE_LANG"),
