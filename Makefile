...
wait-web:    ## ждать готовности веб-админки после make up (до ~15 мин)
	@for i in $$(seq 1 10); do \
	  ...
	  printf "⏳ api ещё не готов… (попытка %d/10, следующая проверка через 90 с)\n" $$i; \
	  sleep 90; \
	done; \
	printf "\033[31m✗ api не ответил за ~15 мин — смотрите docker compose logs api\033[0m\n"; exit 1