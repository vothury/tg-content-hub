...
wait-idle:
	@for i in $$(seq 1 90); do \
		...
		echo "ожидание завершения обработки: $$busy (попытка $$i/90)"; \
		if [ $$((i % 6)) = 0 ]; then docker compose exec -T postgres psql -U content_hub -d content_hub -c "$(BUSY_SQL)"; fi; \
		sleep 10; \
	done; \
